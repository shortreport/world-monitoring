...
$LOGFILE = "$BASE\logs\trump_$TODAY.log"
if (-not (Test-Path "$BASE\logs")) { New-Item -ItemType Directory "$BASE\logs" | Out-Null }

"[$((Get-Date -Format 'yyyy/MM/dd HH:mm:ss'))] START: Trump monitor" | Out-File $LOGFILE -Append -Encoding UTF8

# 1. トランプモニターデータ収集
try {
    & $PYTHON "$BASE\trump_monitor.py" 2>&1 | Out-File $LOGFILE -Append -Encoding UTF8
    if ($LASTEXITCODE -ne 0) {
        "[$((Get-Date -Format 'yyyy/MM/dd HH:mm:ss'))] trump_monitor.py FAILED (RC=$LASTEXITCODE)" | Out-File $LOGFILE -Append -Encoding UTF8
        exit 1
    }
} catch {
    "[$((Get-Date -Format 'yyyy/MM/dd HH:mm:ss'))] trump_monitor.py EXCEPTION: $_" | Out-File $LOGFILE -Append -Encoding UTF8
    exit 1
}

# 2. Type B trump.html 生成
"[$((Get-Date -Format 'yyyy/MM/dd HH:mm:ss'))] Type B trump.html 生成中..." | Out-File $LOGFILE -Append -Encoding UTF8
try {
    & $PYTHON "$BASE\make_en_trump.py" 2>&1 | Out-File $LOGFILE -Append -Encoding UTF8
} catch {
    "[$((Get-Date -Format 'yyyy/MM/dd HH:mm:ss'))] make_en_trump.py FAILED: $_" | Out-File $LOGFILE -Append -Encoding UTF8
}

# 3. Type C trump.html 生成
"[$((Get-Date -Format 'yyyy/MM/dd HH:mm:ss'))] Type C trump.html 生成中..." | Out-File $LOGFILE -Append -Encoding UTF8
try {
    & $PYTHON "$BASE\update_type_c.py" --only trump --no-push 2>&1 | Out-File $LOGFILE -Append -Encoding UTF8
} catch {
...
Set-Location $BASE
& git add docs/ 2>&1 | Out-File $LOGFILE -Append -Encoding UTF8
...
if (-not $changed) {
    ...
    & git commit -m "Trump update: $ts" 2>&1 | Out-File $LOGFILE -Append -Encoding UTF8
