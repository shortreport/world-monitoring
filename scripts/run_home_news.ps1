...
$LOGFILE = "$BASE\logs\home_news_$TODAY.log"
if (-not (Test-Path "$BASE\logs")) { New-Item -ItemType Directory "$BASE\logs" | Out-Null }

"[$((Get-Date -Format 'yyyy/MM/dd HH:mm:ss'))] START: News only" | Out-File $LOGFILE -Append -Encoding UTF8

# --- Type A: ニュース収集 + HTML 生成 + push ---
try {
    & $PYTHON "$BASE\update_home.py" 2>&1 | Out-File $LOGFILE -Append -Encoding UTF8
} catch {
...
if (-not $changed) {
    ...
    & git commit -m "Home news (B/C): $ts" 2>&1 | Out-File $LOGFILE -Append -Encoding UTF8
