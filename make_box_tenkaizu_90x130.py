...
S_tab_lid   = 9
S_long_lid  = T_PLATE*2 + S_tab_lid    # = 10mm
R_lock_lid  = 12
S_short_lid = T_PLATE*2 + R_lock_lid   # = 13mm
...
plt.savefig('box_01_itatorie_90x130.png', dpi=150, bbox_inches='tight', facecolor='white')
plt.close()
print("box_01_itatorie_90x130.png")
...
# ── コーナー処理（4隅）── タブ外端を2mmカット
EC = 2  # 外端カット mm

# 下側2隅：(xi0,xi1,左カット,右カット)
for xi0, xi1, ec0, ec1 in [(xs[0], xs[1], EC, 0), (xs[2], xs[3], 0, EC)]:
    tab_x0 = xi0 + ec0
    tab_x1 = xi1 - ec1
    cx_tab = (tab_x0 + tab_x1) / 2
    cx_cut = (xi0 + xi1) / 2
    ax.add_patch(patches.Rectangle(T2(tab_x0, ys[0]), (tab_x1-tab_x0)*sc, S_long*sc,
                 edgecolor='#BF360C', facecolor=zc['tab'], lw=1.5, zorder=4))
    ax.text(*T2(cx_tab, S_long/2), f'タブ\n{S_long}mm', ha='center', va='center',
    ...
    ax.text(*T2(cx_cut, S_long+H/2), 'x\n切取', ha='center', va='center',
...
for xi0, xi1, ec0, ec1 in [(xs[0], xs[1], EC, 0), (xs[2], xs[3], 0, EC)]:
    ...
    cx_tab = (tab_x0 + tab_x1) / 2
    cx_cut = (xi0 + xi1) / 2
    ax.add_patch(patches.Rectangle(T2(tab_x0, ys[3]-S_long), (tab_x1-tab_x0)*sc, S_long*sc,
                 edgecolor='#BF360C', facecolor=zc['tab'], lw=1.5, zorder=4))
    ax.text(*T2(cx_tab, ys[3]-S_long/2), f'タブ\n{S_long}mm', ha='center', va='center',
            fontsize=6.5, color='white', fontweight='bold', zorder=6)
    ax.add_patch(patches.Rectangle(T2(xi0, ys[2]), (xi1-xi0)*sc, H*sc,
                 edgecolor='red', facecolor='#FFCDD2', lw=1.5, linestyle='--', alpha=0.7, zorder=4))
    ax.text(*T2(cx_cut, ys[2]+H/2), 'x\n切取', ha='center', va='center',
            fontsize=7, color='red', zorder=6)

# ── 外端2mmカット強調表示（4箇所）──
CUT_CLR = '#7B1FA2'  # 紫色で強調
for (cx0, cy0) in [
    (xs[0],       ys[0]),           # 左下
    (xs[3]-EC,    ys[0]),           # 右下
    (xs[0],       ys[3]-S_long),    # 左上
    (xs[3]-EC,    ys[3]-S_long),    # 右上
]:
    # カット部分を濃い斜線ハッチングで表示
    ax.add_patch(patches.Rectangle(T2(cx0, cy0), EC*sc, S_long*sc,
                 edgecolor=CUT_CLR, facecolor='#CE93D8', lw=1.2,
                 hatch='xxx', zorder=9))

# 寸法注釈（下側左右・代表）
for (ax0, ay0, ha_, label_dx, label_dy) in [
    (xs[0],    ys[0],        'right', -3,  5),   # 左下
    (xs[3]-EC, ys[0],        'left',   3,  5),   # 右下
    (xs[0],    ys[3]-S_long, 'right', -3, -5),   # 左上
    (xs[3]-EC, ys[3]-S_long, 'left',   3, -5),   # 右上
]:
    ax.annotate('2mm\nカット', xy=T2(ax0+EC/2, ay0+S_long/2),
                xytext=T2(ax0+EC/2+label_dx, ay0+S_long/2+label_dy),
                fontsize=6, color=CUT_CLR, fontweight='bold', ha=ha_, va='center',
                arrowprops=dict(arrowstyle='->', color=CUT_CLR, lw=1.0), zorder=10)

# ── 外形線（コーナータブ外端2mmカット）──
outline_pts = [
    T2(xs[0]+EC, ys[0]),             # (2,0)     左下タブ外端
    T2(xs[3]-EC, ys[0]),             # (222,0)   右下タブ外端
    T2(xs[3]-EC, ys[0]+S_long),      # (222,10)  右下タブ上端
    T2(xs[2], ys[0]+S_long),         # (157,10)  右下内角
    T2(xs[2], ys[1]),                # (157,65)  右短辺折り線（下）
    T2(xs[3], ys[1]),                # (224,65)  右辺下
    T2(xs[3], ys[2]),                # (224,195) 右辺上
    T2(xs[2], ys[2]),                # (157,195) 右短辺折り線（上）
    T2(xs[2], ys[3]-S_long),         # (157,250) 右上内角
    T2(xs[3]-EC, ys[3]-S_long),      # (222,250) 右上タブ下端
    T2(xs[3]-EC, ys[3]),             # (222,260) 右上タブ外端
    T2(xs[0]+EC, ys[3]),             # (2,260)   左上タブ外端
    T2(xs[0]+EC, ys[3]-S_long),      # (2,250)   左上タブ下端
    T2(xs[1], ys[3]-S_long),         # (67,250)  左上内角
    T2(xs[1], ys[2]),                # (67,195)  左短辺折り線（上）
    T2(xs[0], ys[2]),                # (0,195)   左辺上
    T2(xs[0], ys[1]),                # (0,65)    左辺下
    T2(xs[1], ys[1]),                # (67,65)   左短辺折り線（下）
    T2(xs[1], ys[0]+S_long),         # (67,10)   左下内角
    T2(xs[0]+EC, ys[0]+S_long),      # (2,10)    左下タブ上端
    T2(xs[0]+EC, ys[0]),             # (2,0)     閉じる
]
...
legend_items = [
    ...
    (zc['tab'],    '#BF360C', f'タブ部分 {S_long}mm（T+T+{S_tab}mm）'),
...
plt.savefig('box_02_honkaitenzu_90x130.png', dpi=150, bbox_inches='tight', facecolor='white')
plt.close()
print("box_02_honkaitenzu_90x130.png")
...
plt.savefig('box_03_futakaitenzu_90x130.png', dpi=150, bbox_inches='tight', facecolor='white')
plt.close()
print("box_03_futakaitenzu_90x130.png")
...
for idx, (ax, (title, bg)) in enumerate(zip(axes.flat, steps)):
    ...
    if idx == 0:  # ① 板取り
        ...
        # ── Strip 1/5: 長辺タブ帯（橙）内角4点に2mm切り欠き ──
        brect(0,       0,          xs[1], S_long, '#FF8A65')   # タブ 下左（67mm）
        brect(xs[2],   0,          xs[1], S_long, '#FF8A65')   # タブ 下右（67mm）
        brect(xs[1],   0,             W,  S_long, '#FF8A65')   # タブ 下中央（90mm）
        brect(0,       BH-S_long, xs[1], S_long, '#FF8A65')   # タブ 上左（67mm）
        brect(xs[2],   BH-S_long, xs[1], S_long, '#FF8A65')   # タブ 上右（67mm）
        brect(xs[1],   BH-S_long,    W,  S_long, '#FF8A65')   # タブ 上中央（90mm）
        ...
        # タブラベル
        ax.text(*bpt(xs[1]+W*0.5, S_long*0.5),
                f'タブ帯 {S_long}mm', ha='center', va='center',
                fontsize=6.5, color='white', fontweight='bold', zorder=9)
        # コーナータブサイズ注記
        ax.text(*bpt(xs[1]*0.5-1, S_long*0.5),
                f'65×{int(S_long)}mm', ha='center', va='center',
        ...
        ax.text(5, 0.05,
                f'コーナータブを65×{int(S_long)}mmに（内端2mmカット）　切取りは{int(xs[1])}×{H}mm×4隅',
...
plt.savefig('box_04_tejun_90x130.png', dpi=150, bbox_inches='tight', facecolor='#FAFAFA')
plt.close()
print("box_04_tejun_90x130.png")
...
fig5.tight_layout()
plt.savefig('box_05_haze_detail_90x130.png', dpi=150, bbox_inches='tight', facecolor='white')
plt.close()
print("box_05_haze_detail_90x130.png")

print("\n=== 全図完成 ===")
print("box_01_itatorie_90x130.png    : 板取り図（横並び配置）")
print("box_02_honkaitenzu_90x130.png : 本体展開図（タブ=長辺）")
print("box_03_futakaitenzu_90x130.png: 蓋展開図（タブ=長辺）")
print("box_04_tejun_90x130.png       : 作成手順図（8ステップ）")
print("box_05_haze_detail_90x130.png : ハゼ接合詳細断面図")
